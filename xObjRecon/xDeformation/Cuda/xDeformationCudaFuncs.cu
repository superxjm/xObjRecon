...
#if 0
extern __shared__ float externSumSharedBuf[];
extern __shared__ int externNnzSharedBuf[];
__global__ void CalcResidualSumAndNnzKernel(float *residualSum, int *residualNnz, float *residual)
{
	float *sumSharedBuf = (float *)externSumSharedBuf;
	int *nnzSharedBuf = (int *)externNnzSharedBuf;
...
extern __shared__ float externSharedBuf[];
	...
	int nodeNum)
{
	...
	volatile float *sharedBuf = (float *)externSharedBuf;
