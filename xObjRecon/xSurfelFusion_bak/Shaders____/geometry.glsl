...
vec3 getNormal(vec3 vPosition, vec2 texCoord, float x, float y, vec4 cam, sampler2D depth)
{
	...
	if (vPosition_xf.z > 0.00001 && vPosition_xb.z > 0.00001
	&& vPosition_yf.z > 0.00001 && vPosition_yb.z > 0.00001)
	{
    ...
    /////////
	vPosition_xf = getVertex(vec2(texCoord.x + (1.0 / cols), texCoord.y), x + 1 + 1, y, cam, depth);
    vPosition_xb = getVertex(vec2(texCoord.x - (1.0 / cols), texCoord.y), x - 1 + 1, y, cam, depth);
    
    vPosition_yf = getVertex(vec2(texCoord.x, texCoord.y + (1.0 / rows)), x + 1, y + 1, cam, depth);
    vPosition_yb = getVertex(vec2(texCoord.x, texCoord.y - (1.0 / rows)), x + 1, y - 1, cam, depth);
	...
	/////////
	vPosition_xf = getVertex(vec2(texCoord.x + (1.0 / cols), texCoord.y), x + 1, y + 1, cam, depth);
    vPosition_xb = getVertex(vec2(texCoord.x - (1.0 / cols), texCoord.y), x - 1, y + 1, cam, depth);
    
    vPosition_yf = getVertex(vec2(texCoord.x, texCoord.y + (1.0 / rows)), x, y + 1 + 1, cam, depth);
    vPosition_yb = getVertex(vec2(texCoord.x, texCoord.y - (1.0 / rows)), x, y - 1 + 1, cam, depth);
