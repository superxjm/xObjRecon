...
vec3 getNormal(vec3 vPosition, vec2 texCoord, float x, float y, vec4 cam, sampler2D depth)
{
	...
	if (vPosition_xf.z == 0 || vPosition_xb.z == 0 || vPosition_yf.z == 0 || vPosition_yb.z == 0)
	{
	...
	return -normalize(cross(vPosition_xf - vPosition, vPosition_yb - vPosition) + cross(vPosition_xb - vPosition, vPosition_yf - vPosition));
}
...
vec3 getNormal(vec3 vPosition, vec2 texcoord, int x, int y, vec4 cam, usampler2D depth)
{
	...
	if (vPosition_x.z == 0 || vPosition_y.z == 0)
	{
		return vec3(1000.0f, 1000.0f, 1000.0f);
	}
