...
extern __device__ SiftCameraParams *c_siftCameraParams;
...
{
	//printf("%d %d\n", c_siftCameraParams->m_depthWidth, c_siftCameraParams->m_intensityWidth);
	const unsigned int depthWidth = DEPTH_WIDTH;// c_siftCameraParams->m_depthWidth;
	const unsigned int depthHeight = DEPTH_HEIGHT;// c_siftCameraParams->m_depthHeight;
...
#endif
	{
		...
		int depthx = round((keyLocScale * (float)col + keyLocOffset) * (float)(depthWidth - 1) / (float)(COLOR_WIDTH - 1));
		int depthy = round((keyLocScale * (float)row + keyLocOffset) * (float)(depthHeight-1) / (float)(COLOR_HEIGHT - 1));	
	...
	unsigned int maxNumElements)
{
	...
	if (idx < numInputElements) {
		const float* src = (const float*)d_raw + 4 * idx;
		unsigned short * orientations = (unsigned short*)(&src[3]);
		if (src[2] * keyLocScale >= 3.0f)//c_siftCameraParams->m_minKeyScale)
		{
			if (orientations[0] != 65535) {
				int currFeature = atomicAdd(d_featureCount, 1);
				if (currFeature < maxNumElements) {
...
void __global__  CreateGlobalKeyPointList_Kernel(const float4* d_curFeatureList, float4* d_outKeypointList, unsigned int numInputElements, float keyLocScale, float keyLocOffset, const float* d_depthData)
{
	...
	if (idx < numInputElements) {
		...
		float depthX = posX * (float)(640-1) / (float)(640 -1);
		float depthY = posY * (float)(480-1) / (float)(480 -1);

		const int iposX = round(depthX);
		const int iposY = round(depthY);

		const float depth = d_depthData[iposY * 640 + iposX];	//has to exist -- otherwise something is wrong
