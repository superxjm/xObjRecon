...
void main()
{
    ...
    if(sample.z == 0 || passthrough == 1)
    {
        ...
        vec4 vNormLocal = vec4(getNormal(vPos.xyz, texcoord, int(texcoord.x * cols), int(texcoord.y * rows), cam, rSampler), 1);
		if (vNormLocal.x < 100 || vNormLocal.y < 100 || vNormLocal.z < 100)
		{
			FragColor = vNormLocal;
		}
		else
		{
