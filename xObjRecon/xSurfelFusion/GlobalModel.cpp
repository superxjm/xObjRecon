...
GlobalModel::GlobalModel()
	...
	renderPointCloudRenderBuffer(Resolution::getInstance().width(), 
								 Resolution::getInstance().height(), GL_DEPTH_COMPONENT),
	rgbTextureRender(Resolution::getInstance().width(),
					 Resolution::getInstance().height(),
					 GL_RGBA,
					 GL_RGB,
					 GL_UNSIGNED_BYTE)
{
	renderPointCloudFrameBuffer.AttachColour(*rgbTextureRender.texture);
	renderPointCloudFrameBuffer.AttachDepth(renderPointCloudRenderBuffer);
	...
	const int timeDelta)
{
	...
	//renderPointCloudFrameBuffer.Bind();
	//glPushAttrib(GL_VIEWPORT_BIT);

	//glViewport(0, 0, renderPointCloudRenderBuffer.width, renderPointCloudRenderBuffer.height);
	...
	//renderPointCloudFrameBuffer.Unbind();
	program->Unbind();
	xCheckGlDieOnError();

	//glPopAttrib();
#if 0
	cv::Mat testMat(renderPointCloudRenderBuffer.height, renderPointCloudRenderBuffer.width, CV_8UC3);
	rgbTextureRender.texture->Download(testMat.data, GL_RGB, GL_UNSIGNED_BYTE);
	...
	glPushAttrib(GL_VIEWPORT_BIT);
	glViewport(0, 0, renderPointCloudRenderBuffer.width, renderPointCloudRenderBuffer.height);
	rgbTextureRender.texture->RenderToViewport();
	glPopAttrib();
#endif
}
