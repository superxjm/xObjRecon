...
bool PCA3x3(float3& zAxis, float *data);
...
private:
	GlobalState()
	{
		float weightScale = 100.0;
		m_weightGeo = 0.1		* weightScale;
		m_weightPhoto = 0.00001 * weightScale;
		m_weightReg = 3.0f	* weightScale;
		m_weightRot = 3.0f	* weightScale;

		m_withGeoTerm = true;
		m_withPhotoTerm = false;
		m_withRegTerm = true;
		m_withRotTerm = true;
	}

public:
	bool m_dataTransTest;
	bool m_useStructureSensor;
	bool m_doReconstruction;
	bool m_saveToBinFile;
	bool m_loadFromBinFile;
	bool m_adjustExtrin;
	bool m_debugTracking;
	bool m_useKNN;
	bool m_doNonrigidRegistration;
	float m_minDepthValue;
	float m_maxDepthValue;
	float m_depthThresForEdge;
	float m_nodeNumEachFrag;

	float m_depthFx, m_depthFy, m_depthCx, m_depthCy;
	int m_depthWidth, m_depthHeight;
	float m_colorFx, m_colorFy, m_colorCx, m_colorCy;
	int m_colorWidth, m_colorHeight;
	float m_extrinT[3];
	float m_extrinR[9];

	bool m_withGeoTerm;
	bool m_withPhotoTerm;
	bool m_withRegTerm;
	bool m_withRotTerm;
	float m_weightGeo;
	float m_weightPhoto;
